...
⎿  Plan saved to: ~/.claude/plans/glistening-wibbling-bee.md · /plan to edit
Shagai Harvaа Feature Roadmap

     Current State Summary

     What exists: Game recording (1-4 players, 20 rounds, hit/miss/null), user profiles (Clerk+Convex), clan system (create/invite/stats), dashboard (leaderboard + live stats),
     live spectating, history, admin panel, public share pages, settings (email only).

     Key limitation: Only binary hit/miss per shot. No timing, distance, bone type, or technique data. All analytics computed on-read by scanning up to 2000 games — no precomputed
     stats.
     ...
     Initial rating 1500. Adjust after each game based on opponent strength + score differential. For 3-4 player games, use pairwise comparison (average of all pairwise ELO
     changes).
     ...
     Template-driven insights in FinishedModal: "You won because your accuracy was 85% vs their 60%", "Your strongest stretch was shots 5-12 (100%)". 5-10 insight templates based
     on conditions.
     - Extend components/series/FinishedModal.tsx
     ...
     What: Add the playerStats table definition with fields: userId, totalGames, totalWins, totalHits, totalShots, avgAccuracy, currentStreak, bestStreak, last10Results, rating,
     ratingDeviation, updatedAt. Add indexes: by_user, by_rating.
     ...
     Test: Run from Convex dashboard. Verify playerStats totals match what the old leaderboard shows. Verify gameParticipants count matches total player entries across all finished
      games.
     ...
     Test: Finish a game → tap share button → image generates → native share sheet opens on mobile (or image downloads on desktop). Image should look clean with no rendering
     artifacts.
...
⏺ Let's start with Step 1. First let me read the current schema.

⏺ Read(convex/schema.ts)
⎿  Read 84 lines

✽ Adding `playerStats` table to schema… (esc to interrupt · ctrl+t to hide todos · 9m 0s · ↑ 4.3k tokens)
⎿  ☐ Step 1: Add `playerStats` table to schema
☐ Step 2: Add `gameParticipants` table to schema
☐ Step 3: Add `result` field to `games` table
☐ Step 4: Create stats update logic (convex/stats.ts)
☐ Step 5: Wire stats update into game finish flow
☐ Step 6: Backfill existing games
☐ Step 7: Rewrite dashboard leaderboard to use playerStats
☐ Step 8: Add ELO rating computation
