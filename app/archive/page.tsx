...
export default function ArchivePage() {
  ...
  useEffect(() => {
    if (isDirty) {
      const confirmLeave = window.confirm(
        "Танд хадгалаагүй өөрчлөлт байна. Гарвал мэдээлэл устгах болно. Гарах уу?",
      );
  ...
  const filteredList = useMemo(() => {
    ...
    return savedArchives.filter((arc: any) =>
      arc.startDate.startsWith(viewYear),
    );
  ...
  const createTeam = () => {
    ...
    updateGlobalData({
      ...archiveData,
      teams: [newTeam, ...archiveData.teams],
    });
  };
    ...
    <div className="min-h-screen px-4 py-6 bg-slate-50/50">
      <div className="max-w-md mx-auto">
        <header className="flex items-center justify-between mb-6">
          ...
          {isDirty && (
            <span className="text-[9px] bg-amber-100 text-amber-600 px-2 py-1 rounded-full font-black animate-pulse">
              ХАДГАЛААГҮЙ
            </span>
          )}
        </header>
        ...
        {viewYear && !editingId && !showForm && (
          <div className="space-y-4 mb-20">
            <Button
              ...
              className="w-full h-14 bg-slate-900 text-white rounded-2xl shadow-lg font-bold gap-2"
            >
            ...
            <div className="space-y-3">
              {filteredList.map((arc: any) => (
                <Card
                  key={arc._id}
                  className="p-4 border-slate-200 cursor-pointer group"
                  onClick={() => {
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase">
                        {arc.startDate} • {arc.locationName}
                      </p>
                    </div>
            ...
            >
              ...
              <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (
                        isDirty &&
                        !confirm("Цуцлах уу? Өөрчлөлт устах болно.")
                      )
                        return;
                      setIsDirty(false);
                      setEditingId(null);
                    ...
                    className="h-6 text-[10px] text-red-500 font-bold"
                  >
                    БОЛИХ
                  </Button>
                </div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateGlobalData({ ...archiveData, title: e.target.value })
                  }
                />
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      value={archiveData.organizerName}
                      onChange={(e) =>
                        updateGlobalData({
                          ...archiveData,
                  ...
                  <div className="relative">
                    ...
                    <Input
                      className="pl-9"
                      placeholder="Утас"
                      value={archiveData.organizerContact}
                      onChange={(e) =>
                        updateGlobalData({
                          ...archiveData,
                ...
                <div className="space-y-2 pt-2 border-t">
                  <div className="relative">
                    ...
                    <Input
                      className="pl-9"
                      placeholder="Байршил / Заалны нэр"
                      value={archiveData.locationName}
                      onChange={(e) =>
                        updateGlobalData({
                          ...archiveData,
              ...
              <section className="space-y-4">
                <div className="flex justify-between items-center px-1">
                  <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Багийн бүртгэл ({archiveData.teams.length})
                  </h2>
                  <Button
                    onClick={createTeam}
                    size="sm"
                    className="h-7 bg-slate-900 text-[10px] font-bold rounded-full"
                  >
                    + БАГ НЭМЭХ
                  </Button>
                </div>
                ...
                <div className="space-y-3">
                  {archiveData.teams.map((team, tIdx) => (
                    ...
                    >
                      <div
                        className={`p-2 flex justify-between items-center ${team.isEditing ? "bg-slate-900" : "bg-slate-50"}`}
                      >
                        {team.isEditing ? (
                          <Input
                            className="h-8 bg-transparent text-white border-none font-bold focus-visible:ring-0"
                        ...
                        <div className="flex gap-1">
                          <Button
                            ...
                            className="text-emerald-400 font-bold h-8"
                          >
                            {team.isEditing ? "OK" : "Засах"}
                          </Button>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() =>
                              updateGlobalData({
                                ...archiveData,
                                teams: archiveData.teams.filter(
                                  (t) => t.id !== team.id,
                                ),
                              })
                            }
                      ...
                      {team.isEditing && (
                        <CardContent className="p-4 bg-white space-y-4">
                          <div className="grid grid-cols-3 gap-2">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                nt[tIdx].country = e.target.value;
                          ...
                          <div className="space-y-3 border-t pt-3">
                              ...
                              >
                                <div className="relative">
                                  ...
                                  <Input
                                    ...
                                    placeholder="Харваачийн нэр"
                                    value={p.name}
                                    onChange={(e) => {
                                      const nt = [...archiveData.teams];
                                      nt[tIdx].players[pIdx].name =
                                        e.target.value;
                                      updateGlobalData({
                                        ...archiveData,
                                        teams: nt,
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <div className="relative">
                                    ...
                                    <select
                                      className="w-full pl-8 h-8 text-[10px] bg-white border border-slate-200 rounded-md outline-none"
                                      value={p.degree}
                                      onChange={(e) => {
                                        const nt = [...archiveData.teams];
                                        nt[tIdx].players[pIdx].degree =
                                          e.target.value;
                                        updateGlobalData({
                                          ...archiveData,
                                          teams: nt,
                            ...
                            <Button
                              variant="ghost"
                              className="w-full h-8 text-[10px] border-dashed border text-slate-500"
                              onClick={() => {
                                const nt = [...archiveData.teams];
                                nt[tIdx].players.push({
                                  name: "",
                                  degree: "Цолгүй",
                                  age: "",
                                  country: "Монгол",
              ...
              <section className="space-y-4">
                <RankingCard
                  ...
                  data={archiveData.teamRankings}
                  onChange={(d: any) =>
                    updateGlobalData({ ...archiveData, teamRankings: d })
                  }
                />
                <RankingCard
                  title="Цуваа дүн"
                  icon={<User className="w-4 h-4 text-blue-500" />}
                  data={archiveData.individualRankings}
                  onChange={(d: any) =>
                    updateGlobalData({ ...archiveData, individualRankings: d })
                  }
                />
              </section>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={isSaving || !archiveData.title}
                  className="flex-1 h-16 bg-emerald-600 text-white font-bold text-lg rounded-2xl shadow-xl shadow-emerald-100 transition-transform active:scale-95"
                >
                  <Save className="w-6 h-6 mr-2" />{" "}
                  {isSaving ? "ХАДГАЛЖ БАЙНА..." : "МЭДЭЭЛЛИЙГ ХАДГАЛАХ"}
                </Button>
                {editingId && (
                  <Button
                    variant="destructive"
                    className="h-16 w-16 rounded-2xl shadow-xl shadow-red-100 transition-transform active:scale-95"
                    onClick={async () => {
    ...
    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-4">
      ...
      <div className="space-y-3">
        <RankInput
          ...
          value={data.gold}
          onChange={(v: string) => up("gold", v)}
        />
        <RankInput
          label="Мөнгөн медаль"
          color="text-slate-400"
          value={data.silver}
          onChange={(v: string) => up("silver", v)}
        />
        <RankInput
          label="Хүрэл медаль"
          color="text-orange-600"
          value={data.bronze}
          onChange={(v: string) => up("bronze", v)}
        />
      </div>
    </div>
  );
...
function RankInput({ label, color, value, onChange }: any) {
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        <Medal
          className={`absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 ${color}`}
        />
        <Input
          placeholder="Нэр оруулах..."
          className="pl-9 h-9 text-xs bg-slate-50/50 border-slate-100 focus-visible:ring-emerald-500"
          value={value}
